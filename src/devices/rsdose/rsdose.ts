...
export class RSDose extends RSDevice {
  ...
  set hass(obj: any) {
    ...
    for (const head of this._heads) {
      if ("dose_head" in head) {
        head.dose_head.stock_alert = Number(
          this.get_entity("stock_alert_days").state,
        );
  ...
  _get_val(head, entity_id) {
    const entity = this._hass.states[this.entities[head][entity_id].entity_id];
    return entity.state;
  }

  _render_head(head_id, masterOn) {
    let dose_head = null;
    const new_conf = merge(
      this.config.heads.common,
      this.config.heads["head_" + head_id],
    );

    // Guard: entity states may be unavailable when device is disabled or just re-enabled
    const schedule_entity_id =
      this._heads[head_id]?.entities["schedule_enabled"]?.entity_id;
    const supplement_entity_id =
      this._heads[head_id]?.entities["supplement"]?.entity_id;
    if (
      !schedule_entity_id ||
    ...
    const short_name =
      this._hass.states[supplement_entity_id].attributes.supplement.short_name;
    this.supplement_color[short_name] =
      this.config.heads["head_" + head_id].color;
    ...
    if ("dose_head" in this._heads[head_id]) {
      ...
      dose_head = RSDevice.create_device(
        "redsea-dose-head",
        this._hass,
        new_conf,
        this as any,
  ...
  _render(style = null, substyle = null) {
    ...
    return html` <div class="device_bg">
      ...
      />
      <div class="heads">
        ${Array.from({ length: this.config.heads_nb }, (x, i) => i + 1).map(
          (head) => this._render_head(head, this.masterOn),
        )}
      </div>
      ${this.dosing_queue} ${this._render_elements(this.is_on())}
    </div>`;
  ...
  _editor_head_color(head_id) {
    ...
    const color = rgbToHex(
      "rgb(" + this.config.heads["head_" + head_id].color + ");",
    );
    const shortcuts = this.config.heads["head_" + head_id].shortcut;
    return html`
             <tr>
               <td class="config_color">
<input type="color" id="head_${head_id}-color" value="${color}" @change="${
      this.handleChangedEvent
    }" @input="${this.handleChangedEvent}" list="RedSeaColors" />
                ...
                <label class="tab-label">${i18n._("head")} ${head_id}: ${
                  this._hass.states[
                    this._heads[head_id].entities["supplement"].entity_id
                  ].state
                }</label>
              ...
              <td>
                <input type="text" id="head_${head_id}-shortcut" value="${shortcuts}" @change="${
                  this.handleChangedEvent
                }" ></input>
              </td>
           </tr>`;
    ...
    return html` <hr />
      ...
      <form id="heads_colors">
        <table>
          ...
          ${Array.from({ length: this.config.heads_nb }, (x, i) => i + 1).map(
            (head) => this._editor_head_color(head),
          )}
        </table>
      </form>`;
